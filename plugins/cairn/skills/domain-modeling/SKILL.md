...
[CONTEXT-FORMAT.md](./CONTEXT-FORMAT.md), and apply the "writing" skill to the
definition before you write it: a glossary entry is prose, and a fuzzy one
defeats the point of having a glossary.
...
[ADR-FORMAT.md](./ADR-FORMAT.md), and apply the "writing" skill to the ADR
before you save it. An ADR is read once, years later, by someone with no
context; it earns its keep only if it is clear then.
